...
def run(func_args):
    ...
    matrix_path = './data/' + func_args.market + '/industry_classification.npy'

    start_time = datetime.now().strftime('%m%d/%H_%M_%S')
    if func_args.mode == 'train':
        ...
        if func_args.market == 'CSI100':
            ...
            #CSI300:749 SSE50:3141 NAS90:689
            # test_idx=749
            #3384-242=3142 3142-1=3141
            ...
            assert stocks_data.shape[:-1] == rate_of_return.shape, 'file size error'
            test_idx = 1215
            # nasdaq100&sp500 1259    hs300 & zz500 1215
            market_history = None
            allow_short = False
        ...
        try:
            ...
            for epoch in range(func_args.epochs):
                ...
                if metrics['APR'] > max_apr:
                    print('New Best CR Policy!!!!')
                    max_apr = metrics['APR']
                    torch.save(actor, os.path.join(model_save_dir, 'best_cr-'+str(epoch)+'.pkl'))
                    torch.save(agent_wealth, os.path.join(model_save_dir,'wealth'+str(epoch)+'.pkl'))
                    np.save(f'{func_args.market}_output/wealth'+str(epoch)+'.npy',agent_wealth)
                    times = 0
                else:
                    times += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_msu', dest='msu_bool', action='store_false',default=False)
