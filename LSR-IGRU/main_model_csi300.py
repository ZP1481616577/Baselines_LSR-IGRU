...
data_path = "dataset/hs300_2018_2023_new_1.csv"
root_path = "csi300/"
gpu_id = "3"
stock_num = 140
...
class StockrnnBasicModel(object):
    ...
    def train(self):
        ...
        if not is_LR:
            ...
            c = self.vwap_col
            n = self.T
            df_origin_features["t1_{}".format(c)] = df_origin_features.groupby(
                "kdcode"
            )[c].shift(-1)
            ...
            for i in range(len(df1_kdcode_list)):
                if df1_kdcode_list[i] not in dict_kdcode_sw_kdcode_st_2:
                    dict_kdcode_sw_kdcode_st_2[df1_kdcode_list[i]] = (
                        df1_sw_kdcode_2_list[i]
                    )
            ...
            pool.join()
            matrx = np.array(result)
            df = pd.DataFrame(matrx)
            df.to_csv("data_process/matrx_hs.csv", index=False)
        else:
            df = pd.read_csv("data_process/matrx_hs.csv")
            matrx = df.values
...
epoches_list = [2, 3, 4, 5, 8, 10, 12, 13, 14, 15]
number_of_models = [0, 10]
fun_last(
    root_data_path, T, train_his, epoches_list, P, number_of_models, model_data_path
)
