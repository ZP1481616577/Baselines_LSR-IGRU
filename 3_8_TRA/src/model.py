...
class TRAModel(Model):
    ...
    def train_epoch(self, data_set):
        ...
        for batch in tqdm(data_set, total=max_steps):
            ...
            """
            这段代码实现了最优传输（Optimal Transport）的功能。在这段代码中，使用了Sinkhorn算法来计算样本的分配矩阵P，
            然后通过乘以P来对概率进行正则化。具体来说，通过计算-log(概率)与P的点积的和，然后取平均值，得到正则化项reg。
            最后，将正则化项乘以一个系数lamb，并从损失中减去，以实现最优传输的效果。
            """
            if prob is not None:
...
class CNNTrans(nn.Module):
    def __init__(
        ...
        # Calculate the length of the sequence after the convolution
        self.conv_seq_length = (20 - 16 + 2 * 1) // 8 + 1
        
        # Linear layer to match the embedding dimension
        self.fc = nn.Linear(cnn_out_channels * self.conv_seq_length, embedding_dim)
    ...
    def forward(self, x):
        # x shape: (batch_size, seq_length, input_channels), but Conv1d expects (batch_size, input_channels, seq_length)
        x = x.permute(0, 2, 1)
        # print(x.shape)
        ...
        # Reshape and permute the output for the transformer
        x = x.view(-1, x.size(0), self.output_size)
        x = self.pe(x)

        # Transformer encoder
        x = self.transformer_encoder(x)

        # # Using the output of the last token
        # x = x[-1, :, :]
...
class LSTM_HA(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        #print("LSTM-HA输入X.shape = ", type(X), X.shape)
        ...
        #print("H_n.shape", H_n.shape)
        #[window_len(L), batch_size(B),1]
        scores = self.attn2(torch.tanh(self.attn1(torch.cat([outputs, H_n], dim=2))))
        #print("scores.shape",scores.shape)
        # [batch_size(B),window_len(L)] 获取batch中每一个股票的最终状态与之前所有状态的注意力权重（未softmax）
        scores = scores.squeeze(2).transpose(1, 0)
        #print("scores.shape", scores.shape)
        # [batch_size(B),window_len(L)] softmax,获得真正的注意力权重
        ...
        #print("attn_embed.shape", attn_embed.shape)
        embed = torch.relu(self.bn1(self.linear1(attn_embed)))
        # [batch_size(B), num_stocks, output_dim]
        ...
        class DotProductAttention(nn.Module):
            ...
            def forward(self, queries, keys, values, valid_lens=None):
                ...
                # values的形状：(batch_size，“键－值”对的个数，值的维度d)
                # valid_lens的形状:(batch_size，)或者(batch_size，查询的个数)
                d = queries.shape[-1]
                # 设置transpose_b=True为了交换keys的最后两个维度
                scores = th.bmm(queries, keys.transpose(1,2)) / math.sqrt(d)
                self.attention_weights = th.softmax(scores,dim=-1)
                return th.bmm(self.dropout(self.attention_weights), values)
